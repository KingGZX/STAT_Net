...
class CTRGC(nn.Module):
    def __init__(self, in_channels, out_channels, rel_reduction=8, mid_reduction=1):
        ...
        if in_channels == 3 or in_channels == 9:
            ...
            """
            as talked in paper, firstly utilize linear transformations φ and ψ to reduce feature dimension 
            """
    ...
    def forward(self, x, A=None, alpha=1):
        """
        :param x:               shape of x [N, C, T, V]
        :param A:
        :param alpha:
        :return:
        """

        # conv3 is actually the linear transformation as equation 3 in paper
        x1, x2, x3 = self.conv1(x).mean(-2), self.conv2(x).mean(-2), self.conv3(x)
        x1 = self.tanh(x1.unsqueeze(-1) - x2.unsqueeze(-2))  # equation 4

        # conv4 generates channel-wise topology matrix
...
class Model(nn.Module):
    def __init__(self, in_channels, num_class: list, num_point=22, max_hop=1, drop_out=0, adaptive=True):
